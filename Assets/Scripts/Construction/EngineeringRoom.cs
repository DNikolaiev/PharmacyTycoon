...
using UnityEngine;
using System.Linq;
public class EngineeringRoom : SceneObject, IUpgradable {

    public int startCounter;
    public int upgradedCounter;
    ...
    {
        currentCounter += upgradedCounter;
        if(expand)
        GameController.instance.player.skills.enhancementCounter += upgradedCounter;
        SetSellPrice(2);
    }
    ...
    public void Upgrade(int level, bool chargePlayer=true)
    {
        ...
        if (lvl == 2 && hasJointedObject)
        {
            ...
            currentCounter += upgradedCounter * 2;
            IncreaseStats(++level,chargePlayer);
            lvl++;
        }
    }
    ...
    {
        GameController.instance.player.skills.toxicityReducer -= currentCounter;
        base.SellObject();
    }
    ...
    public override void ConfirmUpgrade(UpgradePanel uPanel) //view
    {
        ...
        uPanel.storage.text = "per level";
        uPanel.production.text = "Enhancement";
        uPanel.currentProduction.text = string.Empty;
        uPanel.upgradedProduction.text = string.Empty;
        uPanel.upgradedStorage.text = " +" + upgradedCounter.ToString();
        uPanel.currentStorage.text = GameController.instance.player.skills.enhancementCounter.ToString() ;
